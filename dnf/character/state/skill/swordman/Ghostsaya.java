...
public class Ghostsaya extends BuffActor {
	private boolean right = true;
	private float delta = 0;
	private RoleAnimation an1 = null;
	//private RoleAnimation an2 = null;
	public Ghostsaya(Character ch, float duringtime) {
		super(ch, 1, new Vector2(100, 0), new Vector2(500, 0), duringtime, true);
		if(!ch.inInstance())
			return;
		right = ch.isRight();
		buffstr = "dnf.character.state.buff.swordman.Saya";
		if(ch.inInstance()) {
			Img img1 = ch.getGame().getImg(SetSwordmanSkill.ghost, "ghostsaya1.img");
			Img img2 = ch.getGame().getImg(SetSwordmanSkill.ghost, "summonarea.img");
			//Img img3 = ch.getGame().getImg(SetSwordmanSkill.saya[1]);
	...
	public void draw(Batch batch, float parentAlpha) {
		...
		//an2.draw(batch, delta, parentAlpha, getX()-an2.getWidth()/2, getY(), right);
